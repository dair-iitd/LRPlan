    ...
    def __init__(self,
        ...
        if OPENAI_API_TYPE == 'azure':
            ...
            self.llm = AzureChatOpenAI(temperature=0,
                    max_tokens=256,
                    azure_deployment=react_llm_name,
                    stop= stop_list)
        else:
...
if __name__ == '__main__':
    ...
    # print(args.set_type, args.model_name, args.output_dir)
    ...
    with get_openai_callback() as cb:
        ...
        for number in tqdm(numbers[:]):
            ...
            if not os.path.exists(os.path.join(f'{args.output_dir}/{args.set_type}/generated_plan_{number}.json')):
                ...
                print(f"FOUND ALREADY EXISTING FILE {os.path.join(f'{args.output_dir}/{args.set_type}/generated_plan_{number}.json')}")
