...
if __name__ == "__main__":
    ...
    with get_openai_callback() as cb:
        for number in tqdm(numbers[:]):
            prev_total_tokens =  cb.total_tokens
            prev_prompt_tokens = cb.prompt_tokens
            prev_completion_tokens = cb.completion_tokens
            prev_total_cost = cb.total_cost
            ...
            while True:
                    ...
                    if os.path.exists(os.path.join(f'{args.output_dir}/{args.set_type}/generated_plan_{number}.json')):
                        print("FILE ALREADY EXISTS ",os.path.join(f'{args.output_dir}/{args.set_type}/generated_plan_{number}.json'))
                        planner_results = "FILE ALREADY EXISTS"
                        break
                    ...
                    try:
                        ...
                        errc += 1
                        if errc >= 3:
                            print("Retried 3 times, skipping this query")
                            planner_results = "Retried 3 times, skipping this query"
                            break
            if planner_results == "FILE ALREADY EXISTS":
                continue
            ...
            result[-1]["Total Tokens"] =  cb.total_tokens - prev_total_tokens
            result[-1]["Prompt Tokens"] = cb.prompt_tokens - prev_prompt_tokens
            result[-1]["Completion Tokens"] = cb.completion_tokens - prev_completion_tokens
            result[-1]["Total Cost (USD)"] = cb.total_cost - prev_total_cost
            # write to json file
