...
class Planner:
    def __init__(self,
        ...
        if model_name in  ['mistral-7B-32K']:
            ...
            self.llm = ChatOpenAI(model_name=model_name, openai_api_key=OPENAI_API_KEY, temperature=1)

        elif "deepseek" in model_name:
            self.llm = ChatOpenAI(
            model_name=model_name,
            openai_api_base="https://api.deepseek.com/v1", 
            openai_api_key="ENTER_DEEPSEEK_API_KEY",
            temperature=0.01
        )
        else:
    ...
    def run(self, text, query, log_file=None) -> str:
        ...
        if self.model_name in ['gemini-1.5-pro']:
            # return str(self.llm.invoke(self._build_agent_prompt(text, query)).content)
            return str(self.llm.generate_content(self._build_agent_prompt(text, query)).text)
        else:
...
class ReactPlanner:
    ...
    def __init__(self,
        ...
        if OPENAI_API_TYPE == 'azure':
            self.react_llm = AzureChatOpenAI(temperature=0,
