...
from feedback_script.travelplanner.tools.flights.apis import Flights
from feedback_script.travelplanner.tools.accommodations.apis import Accommodations
from feedback_script.travelplanner.tools.restaurants.apis import Restaurants
from feedback_script.travelplanner.tools.googleDistanceMatrix.apis import GoogleDistanceMatrix
from feedback_script.travelplanner.tools.attractions.apis import Attractions
import math
import json
import re   
import os
...
city_state_set = open("Code/TravelPlanner/feedback_script/travelplanner/database/background/citySet_with_states.txt",'r').read().split('\n')
...
def is_reasonalbe_visiting_city(question, tested_data):
    ...
    if city_list[0] != city_list[-1]:
        ...
        #CHANGED
        return False, "The trip should be a closed circle. Visitor should reach the starting city at the end of the trip."
    ...
    for idx, city in enumerate(city_list):
        ...
        #Ensuring that the city belong to the destination state
...
def is_valid_transportation(question, tested_data):
    ...
    if (('Self-driving' in transportation_list) and ('Flight' in transportation_list)) or (('Taxi' in transportation_list) and ('Self-driving' in transportation_list)):
        #CHANGED
        return False, "The transportation is conflicting. Flight/Taxi is conflicting with Self-driving."
...
def is_valid_information_in_sandbox(question, tested_data):
    ...
    for i in range(min(question['days'],len(tested_data))):
        ...
        if unit['transportation'] and unit['transportation'] != '-':
            ...
            if 'flight number' in value.lower():
                try:
                    ...
                    #CHANGED
                    return False, "The transportation {} in day {} can not be parsed.".format(value,i+1) + " Correct format looks like 'Flight Number: <flight_no>, from <city_x> to <city_y>, Departure Time: <departure_time>, Arrival Time: <arrival_time>'"
                # print(value)
                ...
                try:
                    if len(flight.data[(flight.data['Flight Number'] == value.split('Flight Number: ')[1].split(',')[0]) & (flight.data['OriginCityName']==org_city) & (flight.data['DestCityName']==dest_city)]) < 1:
                        return False, f"The flight number in day {i+1} is invalid in the sandbox. Transportation not found in the reference data."
                except Exception as e:
                    return False, f"Invalid format: Correct format looks like 'Flight Number: <flight_no>, from <city_x> to <city_y>, Departure Time: <departure_time>, Arrival Time: <arrival_time>'"
                ...
                if 'self-driving' in value.lower():
                    #ADDED
                    if org_city == '-' or dest_city == '-':
                        return False, "The transportation {} in day {} can not be parsed.".format(value,i+1) + " Invalid format: Correct format looks like 'Self-driving, from <city_x> to <city_y>, duration: <duration>, distance: <distance>'"
                    if googleDistanceMatrix.run_for_evaluation(org_city, dest_city, mode='self-driving')['cost'] == None:
                        return False, f"The self-driving in day {i+1} is invalid in the sandbox. Transportation not found in the reference data." 
                else:
                    #ADDED
                    if org_city == '-' or dest_city == '-':
                        return False, "The transportation {} in day {} can not be parsed.".format(value,i+1) + " Invalid format: Correct format looks like 'Taxi, from <city_x> to <city_y>, duration: <duration>, distance: <distance>, Cost: <cost>'"
                    if googleDistanceMatrix.run_for_evaluation(org_city, dest_city, mode='taxi')['cost'] == None:
                        return False, f"The taxi in day {i+1} is invalid in the sandbox. Transportation not found in the reference data."

        if 'breakfast' in unit and unit['breakfast'] and unit['breakfast'] != '-':
            name, city = get_valid_name_city(unit['breakfast'])
            #ADDED
            if name == '-' or city == '-':
                return False, "The restaurant {} in day {} can not be parsed.".format(unit['breakfast'],i+1) + " Invalid format: Correct format looks like '<name_of_restaurant>, <city>'"
            if len(restaurants.data[(restaurants.data['Name'].astype(str).str.contains(re.escape(name))) & (restaurants.data['City'] == city)]) < 1:
                return False, f"The breakfast in day {i+1} is invalid in the sandbox. Restaurant not found in the reference data." 
        # elif 'breakfast' not in unit:
        #     return False, f"No Breakfast Info."
        ...
        if 'lunch' in unit and unit['lunch'] and unit['lunch'] != '-':
            ...
            #ADDED
            if name == '-' or city == '-':
                return False, "The restaurant {} in day {} can not be parsed.".format(unit['lunch'],i+1) + " Invalid format: Correct format looks like '<name_of_restaurant>, <city>'"
            if len(restaurants.data[(restaurants.data['Name'].astype(str).str.contains(re.escape(name))) & (restaurants.data['City'] == city)]) < 1:
                return False, f"The lunch in day {i+1} is invalid in the sandbox. Transportation not found in the reference data."
        # elif 'lunch' not in unit:
        #     return False, f"No Lunch Info."
        ...
        if 'dinner' in unit and unit['dinner'] and unit['dinner'] != '-':
            ...
            #ADDED
            if name == '-' or city == '-':
                return False, "The restaurant {} in day {} can not be parsed.".format(unit['dinner'],i+1) + " Invalid format: Correct format looks like '<name_of_restaurant>, <city>'"
            if len(restaurants.data[(restaurants.data['Name'].astype(str).str.contains(re.escape(name))) & (restaurants.data['City'] == city)]) < 1:
                #CHANGED
                return False, f"The dinner in day {i+1} is invalid in the sandbox. Transportation not found in the reference data."
        # elif 'dinner' not in unit:
        #     return False, f"No Dinner Info."
        ...
        if 'attraction' in unit and unit['attraction'] and unit['attraction'] != '-':
            ...
            for attraction in attractions_list:
                ...
                #ADDED
                if name == '-' or city == '-':
                    return False, "The attraction {} in day {} can not be parsed.".format(attraction,i+1) + " Invalid format: Correct format of an attraction looks like '<name_of_attraction1>, <city>'"
                if len(attractions.data[(attractions.data['Name'].astype(str).str.contains(re.escape(name))) & (attractions.data['City'] == city)]) < 1:
                    return False, f"The attraction {attraction} in day {i+1} is invalid in the sandbox. Attraction not found in the reference data."
        # elif 'attraction' not in unit:
        #     return False, f"No Attraction Info."
        ...
        if 'accommodation' in unit and unit['accommodation'] and unit['accommodation'] != '-':
            ...
            #ADDED
            if name == '-' or city == '-':
                return False, "The restaurant {} in day {} can not be parsed.".format(unit['accommodation'],i+1) + " Invalid format: Correct format looks like '<name_of_accommodation>, <city>'"
            # print(name,city)
            ...
            if len(accommodation.data[(accommodation.data['NAME'].astype(str).str.contains(re.escape(name))) & (accommodation.data['city'] == city)]) < 1:
                return False, f"The accommodation in day {i+1} is invalid in the sandbox. Accommodation not found in the reference data."
        # elif 'accommodation' not in unit:
        #     return False, f"No Accommodation Info."
...
def is_valid_accommodaton(question, tested_data):
    ...
    for unit in consectutive_accommodation:
        ...
        if unit and unit[0] not in  ['-',''] :
            ...
            #ADDED
            if name == '-' or city == '-':
                return False, "The accommodation {} can not be parsed.".format(unit[0]) + " Invalid format: Correct format looks like '<name_of_accommodation>, <city>'"
...
def is_not_absent(question, tested_data):
    ...
    for i in range(min(question['days'],len(tested_data))):
        ...
        if ('from ' in unit['current_city'] or 'to ' in unit['current_city']) and unit['transportation'] in ['','-']:
            return False, f"No transportation in day {i+1} is not allowed. You need a transportation to travel from one city to another."
        
        if ('from ' not in unit['current_city'] and  ' to ' not in unit['current_city']) and unit['attraction'] in ['','-']:
            return False, f"No attraction in day {i+1} is not allowed since you are not travelling from one city to another on day {i+1}."
        ...
        if (unit['breakfast'] in ['','-'] or unit['lunch'] in ['','-'] or unit['dinner'] in ['','-']) and 'from ' not in unit['current_city']:
            return False, f"No meal in day {i+1} is not allowed since you are not travelling from one city to another on day {i+1}."
