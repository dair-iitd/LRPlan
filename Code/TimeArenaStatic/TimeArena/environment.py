...
#TODO
meta_prompt_anera = '''As an AI agent, your objective is to efficiently complete a series of tasks as described. You must adhere to the specific requirements and constraints of each task, including dependencies and timing. Efficiency is key; complete all tasks in the shortest possible time. I will provide instructions regarding actions and objects.
**Action Protocol**:
...
class TimeAnera():
    def __init__(self):
        pass
    def load(self, tasknames):
        self.Tasks = [globals()[name]() for name in tasknames]
        self.taskdescriptions = [i.name for i in self.Tasks]
        alldependencygraphs = json.load(open("Code/TimeArenaStatic/algorithm/dependencygraph.json"))
        self.dependencygraphs = {}
        for task in tasknames:
            self.dependencygraphs[task] = alldependencygraphs[task]
        self.actions = [i[0] for i in self.dependencygraphs.values()]
    def getInstruction(self):
        instruction = meta_prompt_anera
        instruction += f"The maximum time allowed for completing all tasks is {len(self.taskdescriptions)*40} minutes. Please cutoff you answer at the completion of all tasks or before this maximum limit. Note that this time limit is always higher than actually required time."
        instruction += "Tasks:\n"
        for t in self.taskdescriptions:
            instruction += f"**Task {t}**\n"
        instruction += "Valid Actions with time required for completion:\n"
        for action in self.actions:
            for k in action:
                if k[-1] == "*":
                    instruction += f"- {k[:-1]}: {action[k]} minutes\n"
                else:
                    instruction += f"- {k}: {action[k]} minutes\n"
        return instruction
    def getDatapoint(self):
        query = self.getInstruction()
        return query, self.dependencygraphs
